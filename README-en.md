...
Usually, to insert a string to a DB, it's necessary to list all the fields without default values. Oftentimes, many of those fields are not important for the test and their values repeat from one fixture to another, creating unnecessary visual garbage and making the maintenance harder.
...
To inherit a record, first you need to assign a name to the string using `$name`:
...
Names assigned to strings must be unique among all loaded fixture files, as well as they must not interfere with template names.

In another fixture file you need to declare that a certain string inherits an earlier defined record with `$extend`, just like with the templates:
...
It's important to note that string inheritance only works with different fixture files. It's not possible to declare inheritance within one file.
...
When you need to write an expression execution result to the DB and not a static value, you can use `$eval()` construct. Everything inside the brackets will be inserted into the DB as raw, non-escaped data. This way, within `$eval()` you can write everything you would in a regular request.
...
Parameters:
- `body` (mandaotyr) - sets the response body;
- `statusCode` - HTTP-code of the response, the default value is `200`;
- `headers` - response headers.
...
Ises different response strategies, depending on a path of a requested resource.
...
After HTTP request execution you can run an SQL request to DB to check the data changes.
The response can contain several records. Those records are compared to the expected list of records.

#### Request definition

Request is a SELECT that returns any number of strings.

- `dbQuery` - a string that contains an SQL request.
