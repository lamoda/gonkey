...
- [Gonkey: инструмент автоматизации тестирования](#gonkey-инструмент-автоматизации-тестирования)
  ...
  - [Переменные](#переменные)
    - [Способы присвоения](#способы-присвоения)
      ...
      - [Из запроса, пойманного моком](#из-запроса-пойманного-моком)
...
- из запроса, пойманного моком в предыдущем тесте
...
#### Из запроса, пойманного моком

Мок может захватывать значения из приходящего к нему запроса в переменные. Переменные становятся доступны *после* завершения теста, то есть использовать их можно в последующих тестах.

Для этого используется `variablesToSet` в определении мока (на одном уровне со `strategy`). Значения — шаблоны чтения входящего запроса, синтаксис тот же, что у [стратегии template](#template): `{{ .request.Json }}`, `{{ .request.Query "name" }}` и `{{ .request.Header "name" }}`.
...
- name: "следующий тест использует захваченные значения"
  ...
  request: '{"user_id": "{{ $capturedUserId }}"}'
```

Если мок (точнее, определение с `variablesToSet`) вызывается в рамках одного теста несколько раз, по умолчанию сохраняется значение последнего вызова. Чтобы сохранить значение первого вызова, используйте полную форму с `fromCall`:

```yaml
      variablesToSet:
        capturedUserId:
          template: '{{ .request.Json.user.id }}'
          fromCall: first    # first | last (по умолчанию)
```

Особенности:

- в отличие от `variables_to_set` для ответа теста, значения здесь — template-строки, а не пути до полей, и мапа плоская (у запроса нет статус-кода);
- `variablesToSet` можно вешать на вложенные определения композитных стратегий (`uriVary`, `methodVary`, `sequence`, `basedOnRequest`) — например, в `sequence` каждый элемент захватывает свой вызов, что позволяет захватить конкретный по счёту вызов;
- переменные захватываются только из запросов, прошедших `requestConstraints`; отсутствие поля в JSON — ошибка теста;
- давайте каждой захватываемой переменной уникальное имя — если два определения мока пишут в одно имя, какое значение победит, не гарантируется, и тест может стать флаки;
- при действительно конкурентных вызовах одного определения «первый/последний» — это порядок, в котором мок обработал запросы;
- если то же имя присваивается и через `variables_to_set` ответа того же теста, приоритет у значения из ответа.
